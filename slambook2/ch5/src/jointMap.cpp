...
int main(int argc, char **argv) {
   ...
   for (int i = 0; i < 5; i++) {
        ...
        colorImgs.push_back(cv::imread((fmt % "images" % "color" % (i + 1) % "png").str()));
        // use −1 flag to load the depth image
        depthImgs.push_back(cv::imread((fmt % "images" % "depth" % (i + 1) % "pgm").str(), -1));

        double data[7] = {0};
        for (auto &d:data) fin >> d;
        Sophus::SE3d pose(Eigen::Quaternion(data[6], data[3], data[4], data[5]), Eigen::Vector3d(data[0], data[1], data[2]));
